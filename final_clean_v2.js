...
// Board name normalization map (Bengali -> English)
const boardNorm = {
...
function parseQuestions(rawText, format) {
  ...
  if (format === 'bangla') {
    ...
    // Only split on full structured headers: Question N:\n[ID: N]\n[Subject: ...]\n[Chapter: ...]\n[Lesson: ...]\n[Board: ...]
  ...
  for (const block of blocks) {
    ...
    const attrs = {};
    const attrKeys = ['ID', 'Subject', 'Chapter', 'Lesson', 'Board', 'আইডি', 'বিষয়', 'অধ্যায়', 'পাঠ', 'বোর্ড'];
    const ar = /\[([^\]]+)\]/g;
    let m;
    while ((m = ar.exec(qp)) !== null) {
      ...
      if (ci > 0) {
        ...
        // Normalize Bangla attr keys to English
...
// Parse both
const banRaw = fs.readFileSync(path.join(backupDir, 'Ch3_CQ_BANGLA.txt'), 'utf-8');
...
console.log(`Bangla raw: ${banQs.length}, English raw: ${engQs.length}`);

// Clean English artifacts
...
// Remove gibberish lines from each English question's text blocks
...
const cleanEng = engQs;
console.log(`English clean: ${cleanEng.length}`);

// Match by (qn + board)
const banKeyed = new Map();
for (const q of banQs) {
  const k = q.qn + '|' + q.board;
  if (!banKeyed.has(k)) banKeyed.set(k, q);
}
const engKeyed = new Map();
for (const q of cleanEng) {
  const k = q.qn + '|' + q.board;
  if (!engKeyed.has(k)) engKeyed.set(k, q);
}
...
// Drop matches where any part mentions missing content
...
const filteredMatches = matches.filter(m => !hasNoContent(m.ban) && !hasNoContent(m.eng));
const dropped = matches.length - filteredMatches.length;
if (dropped) console.log(`Dropped ${dropped} question(s) with missing content`);
matches.length = 0;
matches.push(...filteredMatches);

console.log(`\nMatched ${matches.length} questions\n`);
...
console.log(`Written: Bangla_new.txt (${bCount}) + English_new.txt (${eCount})`);
console.log(bCount === eCount ? '✓ MATCHED' : '✗ MISMATCH');

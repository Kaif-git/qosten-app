                ...
                try {
                    ...
                    if (Array.isArray(parsedParts)) {
                        parsedParts.forEach(p => {
                            partsText += `${p.label}. ${p.text}\n`;
                            answersText += `${p.label}. ${p.answer || "No answer found"}\n`;
                        });
                    }
                } catch (e) {
                    partsText = item.parts || "No parts found";
                    answersText = item.answer || "No answer found";
                }
                
                textContent += (partsText || "No parts found") + "\n";
                textContent += `Answer:\n${answersText || (item.answer || "No answer found")}\n`;
            }
            textContent += "\n---\n\n";
        });

// Rename BN higher-math chapter 5: সূচক ও সূচকীয় সমীকরণ → দ্বিঘাত সমীকরণ
const API = 'https://questions-api.edventure.workers.dev';
const OLD = 'সূচক ও সূচকীয় সমীকরণ';
const NEW = 'দ্বিঘাত সমীকরণ';
...
async function main() {
  console.log('Fetching উচ্চতর গণিত questions...');
  const all = await fetchAll('উচ্চতর গণিত');
  const toUpdate = all.filter(q => q.chapter === OLD);
  console.log(`Found ${toUpdate.length} questions with chapter "${OLD}"`);

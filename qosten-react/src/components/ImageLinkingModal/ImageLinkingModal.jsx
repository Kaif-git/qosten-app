...
export default function ImageLinkingModal({ question, onClose, onLink }) {
  const { questions } = useQuestions();
  const [searchText, setSearchText] = useState('');
  const [filteredQuestions, setFilteredQuestions] = useState([]);
  const [selectedQuestion, setSelectedQuestion] = useState(null);

  // Get available English CQ questions to link with
  useEffect(() => {
    ...
    if (!searchText.trim()) {
      setFilteredQuestions(englishCQs);
      return;
    }

    const searchLower = searchText.toLowerCase();
    const filtered = englishCQs.filter(q => 
  ...
  }, [searchText, questions, question.id]);
  ...
  const handleLink = () => {
    ...
    onLink({
      ...
      linkedParentId: selectedQuestion.id
    });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          <input
            type="text"
            placeholder="Search by question text, subject, chapter, or lesson..."
            value={searchText}
